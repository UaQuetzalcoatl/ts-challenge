...
type Constructor<T> = T extends () => infer Inst ? Inst : (T extends new (...args: any) => any ? InstanceType<T> : any)
type ArrToUnion<T> = T extends any[] ? Constructor<T[number]> : Constructor<T>

type ExtractProps<Props> = {
  [key in keyof Props]: Props[key] extends { type: infer Type} ? ArrToUnion<Type> : ArrToUnion<Props[key]>
}

declare function VueBasicProps<Props, Data, Comp, Methods>(options: {
  props: Props;
  data: (this: ExtractProps<Props>) => Data;
  computed: Comp & ThisType<Comp & Data>
  methods: Methods & ThisType<{[key in keyof Comp]: Comp[key] extends () => infer R ? R : Comp[key]} & Data & Methods & ExtractProps<Props>>
}): any
