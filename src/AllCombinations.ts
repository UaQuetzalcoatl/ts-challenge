...
type ToUnion<S extends string> = S extends `${infer First}${infer Rest}` ? First | ToUnion<Rest> : ''
type UnionCombination<S extends string, T = S> = T extends S ? `${T}` | `${T}${UnionCombination<Exclude<S, T>>}` : never
type AllCombinations<S extends string> =  UnionCombination<ToUnion<S>>
